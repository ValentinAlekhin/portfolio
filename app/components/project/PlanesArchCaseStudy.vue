...
function themeExample(theme: 'light' | 'dark'): ProjectMedia {
  const landing = media('landing')
  const sources = landing.sources?.[localeCode.value]

  return {
    ...landing,
    id: `landing-${theme}`,
    src: sources?.[theme] ?? landing.src,
    sources: undefined,
    captionKey: `${translationKey.value}.themes.items.${theme}`,
    descriptionKey: `${translationKey.value}.themeExamples.${theme}`,
  }
}
  ...
  >
    ...
    <section class="planes-section section-rule">
      ...
      <div class="site-container planes-about">
        <div class="planes-about__index system-label">
          <span>02</span>
          <span>{{ t(`${translationKey}.media.about.caption`) }}</span>
        </div>
        <ProjectScreenshot :media="media('about')" />
      </div>
    </section>
    ...
    <section class="planes-section section-rule">
      <div class="site-container planes-section-head">
        <p class="planes-label system-label">
          <span>// 02</span>{{ t(`${translationKey}.portfolio.label`) }}
        </p>
    ...
    <section class="planes-section planes-section--deep section-rule">
      <div class="site-container planes-copy-grid">
        <p class="planes-label system-label">
          <span>// 03</span>{{ t(`${translationKey}.content.label`) }}
        </p>
        <div class="planes-prose">
    ...
    <section class="planes-section planes-section--themes section-rule">
      <div class="site-container planes-copy-grid">
        <p class="planes-label system-label">
          <span>// 04</span>{{ t(`${translationKey}.themes.label`) }}
        </p>
        <div class="planes-prose">
      ...
      <div class="site-container planes-theme-examples">
        <ProjectScreenshot :media="themeExample('light')" />
        <ProjectScreenshot :media="themeExample('dark')" />
      </div>
    </section>
    ...
    <section class="planes-section section-rule">
      <div class="site-container planes-mobile-grid">
        <div>
          <p class="planes-label system-label">
            <span>// 05</span>{{ t(`${translationKey}.responsive.label`) }}
          </p>
          <div class="planes-prose">
...
.planes-theme-examples { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); align-items: start; gap: clamp(1rem, 3vw, 3rem); margin-top: clamp(3rem, 6vw, 6rem); }
@media (max-width: 800px) { .planes-theme-examples { grid-template-columns: 1fr; } }
</style>
